...
"""
To transmit our qubit between these nodes we will use quantum channel components. To determine the transmission delay 
for these channels we will define a custom delay model that depends on the length of each channel. Let us set the 
average qubit speed to be 50% of the speed of light in vacuum with a standard deviation of 5%. A custom class that 
models the transmission delay is then:
"""
...
"""
The protocol we need at each node should wait for any incoming qubits on the node's port. When a qubit arrives it should 
measure it in the preferred basis, then directly send it back through the same port.
"""
